	...
	function defineComputed(target, key, userDef) {
		...
		if (typeof userDef === 'function') {
			...
			sharedPropertyDefinition.get = userDef.get
				? shouldCache && userDef.cache !== false ? createComputedGetter(key) : userDef.get
				: noop;
 ...
 */
	...
	function addDirective(el, name, rawName, value, arg, modifiers) {
		(el.directives || (el.directives = []))
			.push({ name: name, rawName: rawName, value: value, arg: arg, modifiers: modifiers });
		el.plain = false;
	}
	...
	function genHandler(name, handler) {
		...
		if (!handler.modifiers) {
			...
			var handlerCode = isMethodPath
				...
				: isFunctionExpression ? 'return (' + handler.value + ')($event)' : handler.value;
			/* istanbul ignore if */
